...
export default function PatientDashboard() {
  ...
  const handleBookingChange = async (field, value) => {
    ...
    if (field === 'doctor_id' || field === 'appointment_date') {
      ...
      const date = field === 'appointment_date' ? value : bookingData.appointment_date

      if (doctorId && date) {
        const slots = await fetchAvailableSlots(doctorId, date)
        // You can store these slots and show them in a dropdown
      }
    }
  }
    ...
    <div className="dashboard-container">
      <div className="dashboard-header">
        <div>
          ...
          <p>Welcome, {userProfile?.profile?.first_name} {userProfile?.profile?.last_name}</p>
        </div>
        <div className="header-actions">
          <button onClick={() => setShowBooking(true)} className="btn-primary">
            Book Appointment
          </button>
          <button onClick={() => navigate('/profile-setup?role=patient')} className="btn-secondary">
            Edit Profile
          </button>
            ...
            <form onSubmit={handleBookAppointment}>
                ...
                >
                  ...
                  {doctors.map((doctor) => (
                    <option key={doctor.id} value={doctor.id}>
                      ...
                      {doctor.specialization?.join(', ')} - ${doctor.consultation_price}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => handleBookingChange('specialty_needed', e.target.value)}
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => handleBookingChange('appointment_time', e.target.value)}
                  required
                />
              </div>
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  onChange={(e) => handleBookingChange('patient_notes', e.target.value)}
                  rows="3"
          ...
          filteredAppointments.map((appointment) => (
            <div key={appointment.id} className="appointment-card">
              ...
              <div className="appointment-actions">
                {appointment.payment_status === 'pending' && appointment.status === 'pending' && (
